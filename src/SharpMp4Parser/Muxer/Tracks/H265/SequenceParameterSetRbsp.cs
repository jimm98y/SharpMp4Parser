...
namespace SharpMp4Parser.Muxer.Tracks.H265
{
    ...
    public class SequenceParameterSetRbsp
    {
        ...
        public SequenceParameterSetRbsp(ByteStream input)
        {
            ...
            bool conformance_window_flag = bsr.readBool("conformance_window_flag");
            ...
            bool sps_sub_layer_ordering_info_present_flag = bsr.readBool("sps_sub_layer_ordering_info_present_flag");
            ...
            bool scaling_list_enabled_flag = bsr.readBool("scaling_list_enabled_flag");
            if (scaling_list_enabled_flag)
            {
                bool sps_scaling_list_data_present_flag = bsr.readBool("sps_scaling_list_data_present_flag");
                if (sps_scaling_list_data_present_flag)
                {
                    skip_scaling_list_data(bsr);
                }
            }
            bool amp_enabled_flag = bsr.readBool("amp_enabled_flag");
            bool sample_adaptive_offset_enabled_flag = bsr.readBool("sample_adaptive_offset_enabled_flag");
            bool pcm_enabled_flag = bsr.readBool("pcm_enabled_flag");
            ...
            if (pcm_enabled_flag)
            {
                ...
                bool pcm_loop_filter_disabled_flag = bsr.readBool("pcm_loop_filter_disabled_flag");
            }
            ...
            bool long_term_ref_pics_present_flag = bsr.readBool("long_term_ref_pics_present_flag");
            if (long_term_ref_pics_present_flag)
            {
                ...
                bool[] used_by_curr_pic_lt_sps_flag = new bool[num_long_term_ref_pics_sps];
            ...
            bool sps_temporal_mvp_enabled_flag = bsr.readBool("sps_temporal_mvp_enabled_flag");
            bool strong_intra_smoothing_enabled_flag = bsr.readBool("strong_intra_smoothing_enabled_flag");
            bool vui_parameters_present_flag = bsr.readBool("vui_parameters_present_flag");
        ...
        private void parse_short_term_ref_pic_sets(int num_short_term_ref_pic_sets, CAVLCReader bsr)
        {
            // Based on FFMPEG implementation -- see hevc.c "parse_rps"
            long[]
          num_delta_pocs = new long[num_short_term_ref_pic_sets];
            for (int rpsIdx = 0; rpsIdx < num_short_term_ref_pic_sets; rpsIdx++)
            {
                if (rpsIdx != 0 && bsr.readBool())
                {
                    ...
                    for (int i = 0; i <= num_delta_pocs[rpsIdx - 1]; i++)
                    {
                        ...
                        bool used_by_curr_pic_flag = bsr.readBool();
        ...
        private static void skip_scaling_list_data(CAVLCReader bsr)
        {
