...
namespace SharpMp4Parser.Muxer.Tracks.H263
{
     ...
     *
     */
    public class H263TrackImpl : AbstractH26XTrack
    {
        ...
        private List<ByteBuffer> esdsStuff = new List<ByteBuffer>();
        private bool esdsComplete = false;
        private int fixed_vop_time_increment = -1;
        private int vop_time_increment_resolution = 0;
        private VisualSampleEntry mp4v;

        public H263TrackImpl(DataSource dataSource) : base(dataSource, false)
        {
            LookAhead la = new LookAhead(dataSource);
            ByteBuffer nal;
            List<ByteBuffer> nalsInSample = new List<ByteBuffer>();
            ...
            while ((nal = findNextNal(la)) != null)
            {
                ...
                {
                    throw new Exception("Got start code I don't know. Ask Sebastian via mp4parser mailing list what to do");
                }
        ...
        private int parse0x05Unit(ByteBuffer nal)
        {
            int visual_object_verid = 0;
            BitReaderBuffer brb = new BitReaderBuffer(nal);
            bool is_visual_object_identifier = brb.readBool();
        ...
        {
            BitReaderBuffer brb = new BitReaderBuffer(nal);
            bool random_accessible_vol = brb.readBool();
            int video_object_type_indication = brb.readBits(8);
            bool is_object_layer_identifier = brb.readBool();
            ...
            bool vol_control_parameters = brb.readBool();
            if (vol_control_parameters)
            {
                int chroma_format = brb.readBits(2);
                bool low_delay = brb.readBool();
                bool vbv_parameters = brb.readBool();
                if (vbv_parameters)
                {
                    throw new Exception("Implemented when needed");
            ...
            {
                throw new Exception("Please implmenet me");
            }
        }
